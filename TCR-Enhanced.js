...
TCREnhancedModel = require('app/base/Class').extend({
    ...
    init: function(){
        ...
        Lang.chat.help = "<strong>Comandos do Chat:</strong><br/>/em &nbsp; <em>Emote</em><br/>/me &nbsp; <em>Emote</em><br/>/clear &nbsp; <em>Limpa todo o chat</em><br/>/cap # &nbsp; <em>Limita o numero de avatars a serem carregados (1-200)</em><br/>/ts # &nbsp; <em>Chat timestamps (12, 24, 0)</em><br />/emoji on (or off) <em>Enable/disable Emojis</em><br /> /strobe on/off &nbsp; <em>Strobe light on/off</em><br /> /rave on/off &nbsp; <em>Lights out on/off</em><br />/close &nbsp; <em>Fecha o script</em> <br /> /Avatar # &nbsp; <em> muda para um avatar de Halloween ( # = 1-13)</em> <br /> /Auto On &nbsp; <em> plugbot load </em> <br /> /Auto Chat &nbsp; <em> Liga o auto chat </em> <br /> /Shake &nbsp; <em> SHAKE </em>"
        ...
        a.append('<div class="chat-update"><i>Tumblrs</i>: <a style="color:#FFFF00" href="http://treta220.tumblr.com/" target="_blank">TRETA220v</a> ~ <a style="color:#FF0000" href="http://nsfw-bruce.tumblr.com/" target="_blank">NSFW-bruce</a></span></div>');
    ...
    initCSS: function() {
        ...
        $('body').attr('style','background: none');
            ...
            + '#button-hd-on {background-image: url("http://plug.dj/_/static/images/en/ButtonHDOn.4dc32b8.png");}'
...
initPopout : function(){
    ...
    onChat: function(data){
        if (data.type == 'message' && (API.hasPermission(data.fromID, API.ROLE.BOUNCER)  || data.fromID == "5105e7a23e083e5100cc1d96") && data.message.indexOf('!strobe on') === 0) {
            API.chatLog(data.from + ' ligou os strobes!');
           require ('app/views/room/AudienceView').strobeMode('true');
        } else if (data.type == 'message' && (API.hasPermission(data.fromID, API.ROLE.BOUNCER)|| data.fromID == "5105e7a23e083e5100cc1d96") && data.message.indexOf('!strobe off') === 0) {
            API.chatLog(dafa.from + ' desligou os strobes!')
            require ('app/views/room/AudienceView').strobeMode();
        } else if (data.type == 'message' && (API.hasPermission(data.fromID, API.ROLE.BOUNCER)  || data.fromID == "5105e7a23e083e5100cc1d96") && data.message.indexOf('!rave on') === 0) {
            API.chatLog(data.from + ' apagou as luzes!');
             require ('app/views/room/AudienceView').lightsOut('true');
        } else if (data.type == 'message' && (API.hasPermission(data.fromID, API.ROLE.BOUNCER)  || data.fromID == "5105e7a23e083e5100cc1d96") && data.message.indexOf('!rave off') === 0) {
